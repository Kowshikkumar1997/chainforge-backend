...
const { verifyOnEtherscan } = require("./services/etherscanVerify");
...
app.post("/create-token", async (req, res) => {
  ...
  try {
    ...
    /* -------------------------------------------------------
       Load encoded constructor args from deployment artifact
    ------------------------------------------------------- */

    const deploymentArtifactPath = deployResult.deploymentPath;
    const deploymentArtifact = JSON.parse(
      fs.readFileSync(deploymentArtifactPath, "utf-8")
    );

    const encodedConstructorArgs =
      deploymentArtifact.constructorArgsEncoded || "";

    /* -------------------------------------------------------
       Initialize deployment record (before verification)
    ------------------------------------------------------- */
    ...
    try {
      verificationResult = await verifyOnEtherscan({
        address: deployResult.address,
        artifactKey,
        constructorArgs, // still needed to recompute ABI types
      });
    } catch (err) {
      verificationResult = {
        success: false,
    ...
    deploymentRecord.verificationStatus = verificationResult.status;
    deploymentRecord.verificationMessage = verificationResult.message || "";

    if (verificationResult.status === "verified") {
      deploymentRecord.etherscanUrl =
        `https://sepolia.etherscan.io/address/${deployResult.address}`;
    }
...
app.post("/create-chain", async (req, res) => {
  try {
    ...
    fs.writeFileSync(path.join(chainDir, "manifest.json"),JSON.stringify(manifest, null, 2));
    
    const readme = `# ${chainName}
    This directory was generated by ChainForge.
    Project Type: Chain Scaffold
    Consensus Mechanism: ${consensusType}
    Modules: ${modules.length ? modules.join(", ") : "none"}
    A machine-readable project manifest is available in: manifest.json
    This file is intended to be consumed by tooling, dashboards, and automation layers.
...
app.post("/verify-contract", async (req, res) => {
  try {
    const { deploymentFile } = req.body;

    if (!deploymentFile) {
      return res.status(400).json({ message: "deploymentFile required" });
    }
    ...
    const guid = await verifyOnEtherscan({
    ...
    console.error("[verify-contract] failed", err);
    return res.status(500).json({ message: err.message });
  }
});
